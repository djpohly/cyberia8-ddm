...
if bUse3dModels == 'On' then
	t[#t+1] = Def.ActorFrame{
		...
		Def.ActorFrame{
			...
			Def.ActorFrame{
				InitCommand=cmd(x,SCREEN_RIGHT-160;y,SCREEN_CENTER_Y;zoom,0.4;rotationx,-120;rotationz,-90;);
...
--[[
	LoadActor("effect")..{
		InitCommand=cmd(x,SCREEN_LEFT+200;CenterY;zoomto,320,320;customtexturerect,0,0,1,SCREEN_HEIGHT/self:GetHeight();ztest,true);
		OnCommand=cmd(texcoordvelocity,0,0.1;diffuse,color("1,1,1,0.3"););
	};
]]
	Def.Quad{
		InitCommand=cmd(CenterX;vertalign,top;y,SCREEN_TOP;zoomtowidth,SCREEN_WIDTH;zoomtoheight,0;diffuse,color("0,0,0,0"););
		OnCommand=cmd(accelerate,0.5;diffuse,color("0,0.7,1,0.35");diffusetopedge,color("0,1,1,1");zoomtoheight,SCREEN_HEIGHT*0.35;);
	};
	
	Def.Quad{
		InitCommand=cmd(CenterX;vertalign,bottom;y,SCREEN_BOTTOM;zoomtowidth,SCREEN_WIDTH;zoomtoheight,0;diffuse,color("0,0,0,0"););
		OnCommand=cmd(accelerate,0.5;diffuse,color("0,0.7,1,0.35");diffusebottomedge,color("0,1,1,1");zoomtoheight,SCREEN_HEIGHT*0.35;);
	};
	
	Def.Quad{
		InitCommand=cmd(Center;zoomtowidth,SCREEN_WIDTH;zoomtoheight,0;diffuse,color("0,0,0,0"););
		OnCommand=cmd(accelerate,0.5;diffuse,color("0,0.7,1,0.35");zoomtoheight,SCREEN_HEIGHT-((SCREEN_HEIGHT*0.35)*2);queuecommand,"Repeat";);
