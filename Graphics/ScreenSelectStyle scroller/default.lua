...
local t = Def.ActorFrame{
	...
	Def.ActorFrame{
		...
		LoadActor("_"..gc:GetName().."_label")..{
			InitCommand=cmd(x,SCREEN_CENTER_X+130;y,SCREEN_CENTER_Y+20+20;vertalign,bottom;);
		...
		LoadActor("_"..gc:GetName().."_explain1")..{
			InitCommand=cmd(x,SCREEN_CENTER_X-104;y,SCREEN_CENTER_Y-88+10);
		...
		Def.Sprite{
			InitCommand=function(self)
				self:x(SCREEN_CENTER_X-118);
				self:y(SCREEN_CENTER_Y-29+10);
				if IsNetConnected() then
					self:Load(THEME:GetPathG("ScreenSelectStyle","scroller/_"..gc:GetName().."_explain3"));
				else
					self:Load(THEME:GetPathG("ScreenSelectStyle","scroller/_"..gc:GetName().."_explain2"));
				end;
			end;
